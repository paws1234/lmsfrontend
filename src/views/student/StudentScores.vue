  ...
  <div class="scores max-w-4xl mx-auto p-4">
    ...
    <div v-for="(formScores, formMapId) in scoresByFormMapId" :key="formMapId" class="mb-6">
      <div class="bg-white shadow-lg rounded-lg p-4">
        ...
        <ul class="space-y-4">
          <li v-for="(score, index) in formScores" :key="index" class="flex justify-between items-center border-b pb-4">
            <div>
              <p class="text-lg font-medium text-gray-600">
                Correct Answer: <span class="font-bold text-blue-600">{{ score.correct_answers_count }}</span> / 
                Perfect Score: <span class="font-bold text-blue-600">{{ score.perfect_score || score.total_submissions }}</span>
              </p>
            ...
            <div class="flex items-center space-x-2">
              ...
              <span v-else class="text-yellow-500 font-semibold" @click="openReviewModal(score.submissions)">Review</span>
            </div>
          </li>
        </ul>
      </div>
